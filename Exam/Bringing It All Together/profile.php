...
if (isset($_POST['text'])) {
    ...
    if ($stmt->rowCount())
        $stmt2 = $pdo->prepare('UPDATE profiles 
                                SET text=:text WHERE user=:user');
    else
        $stmt2 = $pdo->prepare('INSERT INTO profiles 
                                VALUES(:user, :text)');
...
if (isset($_FILES['image']['name'])) {
    ...
    if ($info) {
        list($w, $h, $type) = $info;
        switch ($type) {
            case IMAGETYPE_GIF:     $src = imagecreatefromgif($saveto);
            break;
            case IMAGETYPE_JPEG:    $src = imagecreatefromjpeg($saveto);
            break;
            case IMAGETYPE_PNG:     $src = imagecreatefrompng($saveto);
            break;
...
?>
        ...
        <h3>Enter or edit your details and/or upload an images</h3>
        <textarea
         name="text" cols="50"><?php echo $text_html_entities; ?>
        </textarea>
