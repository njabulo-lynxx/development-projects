...
if (isset($_POST['admin'])) {
    $stmt = $pdo->prepare('SELECT * FROM adminauth WHERE admin = ?');
    $stmt->execute([$_POST['admin']]);

    $admin_html_entities = htmlentities($_POST['admin']);
    if ($stmt->rowCount())
        echo "<span class='taken'>&nbsp; &#x2718; " .
             "The username 'admin_html_entities' is taken</span>";
    else
        echo "<span class='available'>&nbsp; &#x2714; " .
             "The username '$admin_html_entities' is available</span>";
}
